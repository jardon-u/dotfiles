...
if !exists("c_no_ansi") || exists("c_ansi_typedefs")
  syn match   cType		"\<size_t\s\|wchar_t\s\|ptrdiff_t\s\|sig_atomic_t\s\|fpos_t\s\>"
  syn match   cType		"\<\(clock_t\s\|time_t\s\|va_list\s\|jmp_buf\s\|FILE\s\|DIR\s\|div_t\s\|ldiv_t\s\)\>"
  syn match   cType		"\<mbstate_t\s\|wctrans_t\s\|wint_t\s\|wctype_t\s\>"
endif
if !exists("c_no_c99") " ISO C99
...
syn keyword	cStorageClass	static register auto volatile extern const inline restrict
...
syn match	cTypedef	display "\<f_[a-z][a-z0-9_]*\>"
syn match	cTypedef	display "\<t_[a-z][a-z0-9_]*\>"
syn match	cTypedef	display "\<[a-z][a-z0-9_]*_t\>"
syn match 	cFct		display "\w\+\s*(."me=e-2
...
if exists("c_comment_strings")
  ...
  syntax region	cComment	matchgroup=cCommentStart start="/\*!" matchgroup=NONE end="\*/" contains=@cCommentGroup,cCommentStartError,cSpaceError fold
else
  ...
  syn region	cComment	matchgroup=cCommentStart start="/\*!" matchgroup=NONE end="\*/" contains=@cCommentGroup,cCommentStartError,cSpaceError fold
endif
